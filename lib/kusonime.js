...
resolve({ status: _res.status, creator: 'Caliph', result: { judul, thumb, desk, genre: genre.split(': ')[1], status, produser: producer, rate, type, link: link1, total_eps: totaleps.split(': ')[1], durasi: durasi.split(': ')[1], tgl_rilis: tglrilis.split(': ')[1], result: { 360 : [...new Set(result360.replace(/:/g, '').replace(/https\/\//g, 'https://').split('\n'))], 480: [...new Set(result480.replace(/:/g, '').replace(/https\/\//g, 'https://').split('\n'))], 720: [...new Set(result720.replace(/:/g, '').replace(/https\/\//g, 'https://').split('\n'))], 1080: [...new Set(result1080.replace(/:/g, '').replace(/https\/\//g, 'https://').split('\n'))] } } })
