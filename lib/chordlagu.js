...
if (result.count < 0) return { status: 404, creator: 'Caliph', message: "no chords for this song were found", result: null };
res = result.posts[0];
h = {};
h.id = res.id;
h.title = clean2(res.title);
let data2 = await axios.get("http://app.chordindonesia.com/?json=get_post&id=" + h.id);
let result2 = data2.data;
h.content = clean(result2.post.content)
